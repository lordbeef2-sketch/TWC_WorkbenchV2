...
`Sign In via TWC` follows the selected server's configured auth lane. **Authentication ID method** uses the Teamwork Cloud AuthServer `authserver.properties` Application ID(s) flow (`/authentication/authorize`, `/authentication/api/token`, and `X-Auth-Secret`). **OpenID** is available for 2024x-style OpenID setup using `/authentication/.well-known/openid-configuration`, `/authentication/authorize`, `/authentication/api/token`, and `X-Auth-Secret` unless an explicit server override says otherwise. **OAuth** is reserved for OAuth 2.0 client setup and stays separate from OSLC/RealSwagger consumer configuration. `Use TWC Token` remains the explicit fallback.
...
- `TWC_AUTH_CLIENT_SECRET`: legacy/bootstrap fallback for the AuthServer client secret. Authentication ID and default OpenID send this as `X-Auth-Secret`; OAuth 2.0 sends it with HTTP basic auth.
...
- `TWC_OIDC_TOKEN_AUTH_METHOD`: OpenID lane token auth method; defaults to `x_auth_secret`. Use `client_secret_basic` only when that exact client registration requires it.
