...
`Sign In via TWC` follows the selected server profile's auth lane. Authentication ID method uses Teamwork Cloud AuthServer `authserver.properties` Application ID(s), `/authentication/authorize`, `/authentication/api/token`, and `X-Auth-Secret`. OpenID is 2024x-only and defaults to `/authentication/.well-known/openid-configuration`, `/authentication/authorize`, `/authentication/api/token`, and `X-Auth-Secret`. OAuth 2.0 uses the Teamwork Cloud Admin OAuth client ID/secret with the Workbench callback `/api/auth/callback`; register the Workbench Public URL plus that callback path in the TWC OAuth client. `Use TWC Token` remains the explicit fallback.
